...
class Quaternion:
    ...
    @overload
    def __mul__(self, other: tuple[float, float, float]) -> tuple[float, float, float]: ...
    ...
    def Euler(x: float, y: float, z: float) -> Quaternion:
        ...
        _x = cx * sy * sz + sx * cy * cz
        _y = cy * sx * sz - cx * cz * sy
...
def _cross_vectors(
    ...
    x = vec0[2] * vec1[1] - vec0[1] * vec1[2]
    y = vec0[0] * vec1[2] - vec0[2] * vec1[0]
    z = vec0[1] * vec1[0] - vec0[0] * vec1[1]
